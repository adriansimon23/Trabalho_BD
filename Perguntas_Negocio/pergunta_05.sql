...
SELECT
    cliente,
    total_de_pedidos,
    -- Cria a posição do cliente no ranking
    RANK() OVER (ORDER BY total_de_pedidos DESC) AS posicao_no_ranking,
    -- Indica se está no TOP 5
    CASE 
        WHEN RANK() OVER (ORDER BY total_de_pedidos DESC) <= 5 THEN 'TOP 5'
        ELSE 'Fora do TOP 5'
    END AS status_top_5
FROM
    pedidos_por_cliente
